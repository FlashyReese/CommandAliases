...
public class ServerReassignCommandBuilder extends AbstractReassignCommandBuilder<ServerCommandSource> {
    ...
    public ServerReassignCommandBuilder(CommandAlias command, Field literalCommandNodeLiteralField, CommandAliasesProvider commandAliasesProvider) {
        super(command, literalCommandNodeLiteralField, commandAliasesProvider, CommandType.SERVER);
    }
    ...
    protected LiteralArgumentBuilder<ServerCommandSource> reassignCommand(CommandDispatcher<ServerCommandSource> dispatcher) {
        if (this.reassignCommand(this.command, dispatcher)) {
            ...
            if (this.command.getCommandMode() == CommandMode.COMMAND_REASSIGN_AND_ALIAS) {
                ...
                return new ServerCustomCommandBuilder(this.command.getCustomCommand(), this.commandAliasesProvider).buildCommand(dispatcher);
            }
        }
        return null;
    }
