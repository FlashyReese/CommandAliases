...
public abstract class AbstractCommandAliasesProvider<S extends CommandSource> {
     ...
     */
    protected void registerCommandAliasesCommands(CommandDispatcher<S> dispatcher) {
        dispatcher.register(this.literal(this.rootCommand).requires(Permissions.require("commandaliases", 4))
                .executes(context -> {
                    ...
                    modContainerOptional.ifPresent(modContainer -> this.sendFeedback(context.getSource(), new LiteralText("Running Command Aliases")
                            .formatted(Formatting.YELLOW)
                            .append(new LiteralText(" v" + modContainer.getMetadata().getVersion()).formatted(Formatting.RED))
                            .formatted(Formatting.RESET)
                            .append(new LiteralText(", "))
                            .append(new LiteralText("Click here to visit the wiki.").formatted(Formatting.UNDERLINE, Formatting.AQUA).styled(style -> style.withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://wiki.commandaliases.flashyreese.me/"))))
                    ));
                        ...
                        .then(this.literal("get").requires(Permissions.require("commandaliases.database.get", 4))
                                .then(this.argument("key", StringArgumentType.string())
                                        .executes(context -> {
                                            ...
                                            String value = this.getDatabase().read(key);
                                            if (value != null) {
                                                this.sendFeedback(context.getSource(), new LiteralText(value));
                                            }
                                            return Command.SINGLE_SUCCESS;
                                        })
                                )
                ...
                .then(this.literal("reload").requires(Permissions.require("commandaliases.reload", 4))
                        .executes(context -> this.commandAliasesReload(context, dispatcher))
                )
                .then(this.literal("load").requires(Permissions.require("commandaliases.load", 4))
                        .executes(context -> this.commandAliasesLoad(context, dispatcher))
                )
                .then(this.literal("unload").requires(Permissions.require("commandaliases.unload", 4))
                        .executes(context -> this.commandAliasesUnload(context, dispatcher))
                )
        );
    }
    ...
    protected abstract int commandAliasesLoad(CommandContext<S> context, CommandDispatcher<S> dispatcher);

    protected abstract int commandAliasesUnload(CommandContext<S> context, CommandDispatcher<S> dispatcher);
