...
public class ClientReassignCommandBuilder extends AbstractReassignCommandBuilder<FabricClientCommandSource> {
    ...
    public ClientReassignCommandBuilder(CommandAlias command, Field literalCommandNodeLiteralField, CommandAliasesProvider commandAliasesProvider) {
        super(command, literalCommandNodeLiteralField, commandAliasesProvider, CommandType.CLIENT);
    }
    ...
    protected LiteralArgumentBuilder<FabricClientCommandSource> reassignCommand(CommandDispatcher<FabricClientCommandSource> dispatcher) {
        if (this.reassignCommand(this.command, dispatcher)) {
            ...
            if (this.command.getCommandMode() == CommandMode.COMMAND_REASSIGN_AND_CUSTOM) {
                return new ClientCustomCommandBuilder(this.command.getCustomCommand(), this.commandAliasesProvider).buildCommand(dispatcher);
            }
        }
        return null;
    }
