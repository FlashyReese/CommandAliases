...
public class CommandAliasesPlaceholders {
    public static void register(ServerCommandAliasesProvider serverCommandAliasesProvider) {
        FunctionProcessor<CommandSourceStack> functionProcessor = new FunctionProcessor<>(serverCommandAliasesProvider);

        functionProcessor.getFunctionMap().forEach((key, value) -> Placeholders.registerServer(Identifier.fromNamespaceAndPath(MOD_ID, key), ((context, argument) -> PlaceholderResult.value(value.apply(context.commandSourceStack(), argument)))));
    }
}
