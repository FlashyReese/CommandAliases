...
public class ClientCommandAliasesProvider<S extends SharedSuggestionProvider> extends AbstractCommandAliasesProvider<S> {
    public ClientCommandAliasesProvider(Field literalCommandNodeLiteralField) {
        super(CommandAliasesMod.platform().configDirectory().resolve("commandaliases-client"), literalCommandNodeLiteralField, "commandaliases:client", CommandType.CLIENT);
    }

    @Override
    protected void sendFeedback(S source, Component text) {
        CommandAliasesMod.platform().sendClientFeedback(source, text);
    }
    ...
    @Override
    protected LiteralArgumentBuilder<S> buildCustomCommand(String filePath, CustomCommand customCommand, AbstractCommandAliasesProvider<S> abstractCommandAliasesProvider, CommandBuildContext registryAccess, CommandDispatcher<S> dispatcher) {
